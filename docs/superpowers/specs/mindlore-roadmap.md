...
## v0.6.3 — Search Engine Overhaul ✅ RELEASED (2026-05-01)
...
## v0.6.4 — Search Cleanup + Code Quality ✅ RELEASED (2026-05-01)
...
## v0.6.5 — Security Hardening + Dead Code Cleanup ✅ RELEASED (2026-05-02)

npm@0.6.5 yayında. 34 commit, 92 suite, 705 test, 0 lint error.

- 13 security fix (SEC-1 → SEC-13): command injection, path traversal, SSRF, TOCTOU, file permissions, atomic write
- input-validation modülü (sanitizeForExecFile, validatePath, validateUrl)
- Dead code cleanup: hybrid-search.ts kaldırıldı, 5 test dosyası refactored
- Simplify bulgular: _searchFts helper, cleanup throttle, extractKeywords unify, STOP_WORDS test
- DRY refactor: isInsideMindloreDir + extractMindloreBaseDir shared helpers

## v0.6.6 — Quality + Compression ✅ RELEASED (2026-05-02)

96 suite, 737 test. Extraction templates, /mindlore-stats, episode dedup, stale filter, search snippets.

| # | Başlık | Durum |
|---|--------|-------|
| Q1 | Session-focus nesting flatten — getEpisodeStats + checkStaleContent extract | ✅ |
| Q2 | Init merge nesting — countMindloreHooks extract | ✅ |
| Q3 | Output Compression — episode stale filter (7d), inject dedup (episode_inject_log), changed files truncation (>10), search content snippets | ✅ |
| Q4 | Source-Type Extraction — 5 extraction template, init copy, ingest template loading | ✅ |
| Q5 | /mindlore-stats skill — session context katkı/maliyet görünürlüğü | ✅ |
| Q6 | Centralized ALL_MIGRATIONS + schema version warning | ✅ |

### Ertelenen Refactor'lar (v0.6.7+)
- loadDbContent 7 param → options object
- buildSessionPayload 6 param → options bag
- CAST(rowid AS TEXT) stringly-typed → episode_inject_log.episode_id INTEGER
- episode_inject_log unbounded growth → TTL/cleanup

---

## v0.6.7 — Intelligence + Refactor

| # | Başlık | Kaynak | Efor |
|---|--------|--------|------|
| Q1 | Lesson Graduation Pipeline — otomatik reflect tetikleme (N episode biriktikten sonra) | v0.7 #14 | Orta |
| Q2 | Graduate/reject tracking — nomination'lara graduated_at/rejected_at + rejection_reason | v0.7 #14 | Orta |
| Q3 | Lesson auto-load — session start'ta aktif projeye ait graduated lesson'ları inject et | v0.7 #14 | Orta |
| R1 | loadDbContent parameter sprawl — 7 positional param → options object | v0.6.6 simplify | Küçük |
| R2 | buildSessionPayload parameter sprawl — 6 param → options bag `{ latestDeltaContent?, sessionId? }` | v0.6.6 simplify | Küçük |
| R3 | episode_inject_log.episode_id stringly-typed — CAST(rowid AS TEXT) → INTEGER schema change | v0.6.6 simplify | Küçük |
| R4 | episode_inject_log unbounded growth — TTL/cleanup mekanizması (session-end veya 30d TTL) | v0.6.6 simplify | Küçük |

---

## v0.6.8 — Infrastructure Prep

| # | Başlık | Kaynak | Efor |
|---|--------|--------|------|
| Q1 | Model Upgrade Değerlendirmesi — multilingual-e5-small → benchmark-driven upgrade | v0.7 #9 | Düşük |
| Q2 | Daemon → MCP Geçişi — mevcut daemon altyapısını MCP-ready hale getir | v0.7 #11 | Orta |
| R1 | 2x COUNT(*) query birleştirme — checkReflectTrigger + getGraduatedLessonCount → tek GROUP BY | v0.6.7 simplify | Düşük |
...
## v0.6.9 — Plugin Polish

| # | Başlık | Kaynak | Efor |
|---|--------|--------|------|
| Q1 | Plugin v2 — dağıtım kanalı güçlendirme, manifest güncellemeleri | v0.7 #16 | Orta |
| Q2 | Stabilizasyon — v0.7 ön koşullarını tamamla, genel polish | - | Düşük |

---

## v0.7 — Capability Expansion (Finale)

Flagship: MCP Server + Knowledge Graph. Diğer maddeler v0.6.6-v0.6.9'a taşındı.

**v0.6.6'dan taşınan (v0.7 MCP Server scope'unda):**
- Handoff Pattern — SessionStart minimal inject + FTS5 on-demand retrieval. MCP Server'da tool-based retrieval ile doğal çözülür. Ön koşul: FTS5'te decisions/episodes/learnings indexed mi doğrulama + agent routing instruction mekanizması.
...
### 9. Model Upgrade Değerlendirmesi (→ v0.6.8 Q1'e taşındı)
...
### 10. Plugin Manifest v2 (→ v0.6.9 Q1'e taşındı)
...
### 11. Daemon → MCP Geçişi (Deprecated) (→ v0.6.8 Q2'ye taşındı)
...
### 14. Episodic → Lesson Graduation Pipeline (Agentic Stack + GBrain İlhamlı) (→ v0.6.7'ye taşındı)
...
### 15. Source-Type Aware Extraction (LLM Wikid İlhamlı) (→ v0.6.6 Q4'e taşındı)
...
### 17. Output Compression / Terse Mode (Context-Mode İlhamlı) (→ v0.6.6 Q3'e taşındı)
