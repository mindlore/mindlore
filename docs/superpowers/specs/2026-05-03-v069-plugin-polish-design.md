...
## D3. Search Hook Profiling (RESOLVED)

### Problem
`hooks/mindlore-search.cjs` — ~500ms ortalama latency. Hipotez: CPU-bound (FTS5 + RRF + proximity scoring + snippet extraction).

### Profiling Sonuçları (2026-05-04)

`npm run perf` çıktısı:

```
mindlore-search (39 calls, 0 errors)
  p50: 31ms  p95: 57ms  p99: 195ms  mean: 38ms
```

### Karar

**p95 < 500ms → Kabul edilebilir.** Hipotez yanlış — gerçek latency çok daha düşük. Optimizasyon gerekmez.

**Kapsam dışı:** p99 = 195ms olabilir ama nadir outlier. v0.7 MCP Server cache layer ile otomatik olarak düşecek. Şu an için action yok.
