...
var require_secure_io = __commonJS({
  "dist/scripts/lib/secure-io.js"(exports2) {
    ...
    exports2.safeMkdir = safeMkdir2;
    exports2.safeWriteFile = safeWriteFile2;
    exports2.safeWriteJson = safeWriteJson;
    var fs_1 = __importDefault(require("fs"));
    function safeMkdir2(dirPath) {
      fs_1.default.mkdirSync(dirPath, { recursive: true, mode: 448 });
    }
    function safeWriteFile2(filePath, data) {
      fs_1.default.writeFileSync(filePath, data, { encoding: "utf8", mode: 384 });
    }
    function safeWriteJson(filePath, obj) {
      safeWriteFile2(filePath, JSON.stringify(obj, null, 2) + "\n");
    }
  }
});
...
var require_secure_io2 = __commonJS({
  "hooks/src/lib/secure-io.cjs"(exports2, module2) {
    ...
    module2.exports = { safeMkdir: safeMkdir2, safeWriteFile: safeWriteFile2, safeWriteJson };
  }
});
...
var require_mindlore_common = __commonJS({
  "hooks/src/lib/mindlore-common.cjs"(exports2, module2) {
    ...
    var { safeMkdir: safeMkdir2, safeWriteFile: safeWriteFile2 } = require_secure_io2();
    var MINDLORE_DIR = ".mindlore";
    var DB_NAME2 = "mindlore.db";
    var SKIP_FILES2 = /* @__PURE__ */ new Set(["INDEX.md", "SCHEMA.md", "log.md"]);
    function globalDir2() {
      if (process.env.MINDLORE_HOME) return process.env.MINDLORE_HOME;
      return path2.join(os.homedir(), MINDLORE_DIR);
    }
    var GLOBAL_MINDLORE_DIR = globalDir2();
    function findMindloreDir() {
      const gDir = globalDir2();
    ...
    function getAllDbs() {
      const dbPath = path2.join(globalDir2(), DB_NAME2);
    ...
    function resolveProject2(ftsProject, filePath, cwdFallback) {
    ...
    function extractFtsMetadata2(meta, body, filePath, baseDir) {
    ...
    function insertFtsRow2(db, entry) {
      const vals = [
        entry.path || "",
        entry.slug || "",
        entry.description || "",
    ...
    function getAllMdFiles(dir, skip) {
      const skipSet = skip || SKIP_FILES2;
      const results = [];
    ...
    function readHookStdin2(fields) {
      let input = "";
      try {
...
CREATE TABLE IF NOT EXISTS episodes (
  ...
  `).all(opts.project, modifier, limit);
    ...
    function _rotateFile(filePath, maxBytes, keepLines) {
      try {
        ...
        if (stat.size > maxBytes) {
          ...
          safeWriteFile2(tmpPath, lines.slice(-keepLines).join("\n") + "\n");
    ...
    function _writeTelemetry({ hookName, duration_ms, ok, extra }) {
      try {
        if (!_telDirEnsured) {
          safeMkdir2(GLOBAL_MINDLORE_DIR);
          _telDirEnsured = true;
        }
    ...
    module2.exports = {
      ...
      extractFtsMetadata: extractFtsMetadata2,
      readHookStdin: readHookStdin2,
      SQL_FTS_CREATE,
      SQL_FTS_INSERT,
      ...
      resolveProject: resolveProject2,
      DEFAULT_MODELS,
      // Episodes (v0.4.1)
      EPISODE_KINDS,
      ...
      _rotateFile,
      isInsideMindloreDir: isInsideMindloreDir2,
      extractMindloreBaseDir: extractMindloreBaseDir2,
      // Telemetry (v0.6.0)
    ...
    function hookLogPath() {
      return path2.join(globalDir2(), "diary", "_hook-log.jsonl");
    }
...
var { DB_NAME, SKIP_FILES, sha256, openDatabase, parseFrontmatter, extractFtsMetadata, insertFtsRow, readHookStdin, getProjectName, resolveProject, globalDir, hookLog, withTelemetry, isInsideMindloreDir, extractMindloreBaseDir } = require_mindlore_common();
