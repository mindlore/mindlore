...
var require_mindlore_common = __commonJS({
  "hooks/src/lib/mindlore-common.cjs"(exports2, module2) {
    ...
    function getLatestDelta(diaryDir) {
      ...
      const deltas = fs.readdirSync(diaryDir).filter((f) => f.startsWith("delta-") && f.endsWith(".md")).sort().reverse();
...
CREATE TABLE IF NOT EXISTS episodes (
  ...
  `).all(opts.project, modifier, limit);
    ...
    function listSnapshots(diaryDir) {
      ...
      return fs.readdirSync(diaryDir).filter((f) => f.startsWith("delta-") || f.startsWith("compaction-")).sort();
    }
    ...
    function getUnpromotedRawFiles(baseDir) {
      ...
      const sourceNames = fs.existsSync(sourcesDir) ? new Set(fs.readdirSync(sourcesDir).filter((f) => f.endsWith(".md"))) : /* @__PURE__ */ new Set();
...
var { findMindloreDir, readHookStdin, hookLog, withTelemetry } = require_mindlore_common();
var SIGNALS_TR = [
  "karar verdik",
  "karar verildi",
  "kararlastirdik",
